...
const ReportComment = ({ taskId, is_project_manager }) => {
    ...
    const handleSendComment = () => {
        ...
        axios
            .post(`/api/reports/${taskId}/comments/${user.id}`, {
                comment: newComment,
    ...
    const handlePinComment = commentId => {
        axios
            .post(`/api/reports/comments/${commentId}/pin`)
            .then(() => {
                fetchComments()
            })
            .catch(() => {
                console.warn('Failed to pin the comment')
            })
    }
    ...
    const handlePinUnComment = commentId => {
        axios
            .post(`/api/reports/comments/${commentId}/unpin`)
            .then(() => {
                fetchComments()
            })
            .catch(() => {
                console.warn('Failed to unpin the comment')
            })
    }
    ...
    const handleDeleteComment = commentId => {
        axios
            .delete(`/api/reports/delete/${commentId}/${user.id}`)
            .then(() => {
                fetchComments()
            })
            .catch(() => {
        ...
        <>
                ...
                <div id="pinned-box" className="rounded-md">
                    <div className="pinned-section pb-3 rounded-md max-h-[270px] scrollbar-hide overflow-y-auto ">
                        {pinnedComments
                            .slice(
                                0,
                                isPinnedCollapsed ? 1 : pinnedComments.length,
                            )
                            .map((comment, index) => (
                                <div
                                    key={index}
                                    className="box-pin mt-6 mx-4 pinned-comment relative flex flex-col w-auto items-start gap-3 shadow-lg p-4 rounded-md"
                                >
                                    ...
                                    {comment.user.id === user.id && (
                                        <button
                                            ...
                                            className="btn-delete absolute top-[-12px] right-1"
                                        >
                                            <svg
                    ...
                    {pinnedComments.length > 1 && (
                        <div className="w-full relative">
                            ...
                            <button
                                ...
                                className={`btn-show-pin m-auto text-sm`}
                            >
                                <svg
                                    className={` ${isPinnedCollapsed ? null : 'isshow'}`}
                                    xmlns="http://www.w3.org/2000/svg"
                                    height="24px"
                                    viewBox="0 -960 960 960"
                                    width="24px"
            ...
            <div id="report-comment" className="py-2 px-2 ">
                ...
                >
                     ...
                     comments && comments.length > 0 ? (
                        comments.map((comment, index) => (
                            ...
                            >
                                ...
                                <button
                                    onClick={() =>
                                        handlePinComment(comment.comment_id)
                                    }
                                    className="btn-pin absolute top-2 right-10"
                                >
                                    <svg
                                ...
                                {comment.user.id == user.id && (
                                    <>
                                        <button
                                            ...
                                            className="btn-delete absolute top-2 right-2"
                                        >
                                            <svg
                                                className=" p-1 text-[50px]"
                                                xmlns="http://www.w3.org/2000/svg"
                    ...
                    {/* ref để đánh dấu vị trí kết thúc danh sách bình luận */}
                </div>
