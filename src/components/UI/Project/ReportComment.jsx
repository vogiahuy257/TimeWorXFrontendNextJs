...
const ReportComment = ({ user,taskId, is_project_manager }) => {
    ...
    const fetchComments = () => {
        axios.get(`/api/reports/${taskId}/comments/${user.id}`)
            .then((response) => {
    ...
    const handleSendComment = () => {
        ...
        axios.post(`/api/reports/${taskId}/comments/${user.id}`, { comment: newComment, is_project_manager: is_project_manager })
            .then(() => {
                setNewComment('')
                fetchComments()
            })
    ...
    const handleDeleteComment = (commentId) => {
        axios.delete(`/api/reports/delete/${commentId}/${user.id}`)
            .then(() => {
                fetchComments()
            })
            .catch(() => {
