...
const DeletedProjectsForm = ({ resetPage, onClose }) => {
    ...
    const fetchDeletedProjects = () => {
        axios
            .get(`/api/projects/deleted/${user.id}`)
            .then(response => {
                setDeletedProjects(response.data)
            })
            .catch(error => {
                toast.error(
                    `Error fetching deleted projects: ${error.response ? error.response.data : error.message}`,
                )
            })
    }
    useEffect(() => {
    ...
    const restoreProject = projectId => {
        axios
            .get(`/api/projects/restore/${projectId}`)
            .then(response => {
    ...
    const permanentlyDeleteProject = projectId => {
        axios
            .delete(`/api/projects/permanently-delete/${projectId}`)
            .then(response => {
                setDeletedProjects(prev =>
                    prev.filter(p => p.project_id !== projectId),
                )
                    ...
                    {deletedProjects.map(project => (
                        <li key={project.project_id}>
                            ...
                            <div className="box-button">
                                <button
                                    onClick={() =>
                                        restoreProject(project.project_id)
                                    }
                                >
                                    <svg
                                        width="18"
