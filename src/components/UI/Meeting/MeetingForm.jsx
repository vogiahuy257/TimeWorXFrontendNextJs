...
const MeetingForm = ({ styles, onClose, meeting, getData }) => {
    ...
    useEffect(() => {
        ...
        if (meeting) {
            ...
            setFormData({
                ...
                user_ids: Array.isArray(meeting.users)
    ...
    return (
        <div
            id="form-meeting"
            className={`${styles} fixed flex w-full h-full top-0 left-0 rounded-lg`}
        >
            <div className="main h-[90%] p-4 max-w-[800px] flex flex-row-reverse rounded-md relative w-[80%] m-auto">
                <button
                    className="btn-close pt-4 px-1 pb-1 rounded-b-full absolute top-0 right-4 "
                    onClick={onClose}
                >
                    <svg
                        width="22"
                ...
                <div className="w-1/3 gap-4 flex flex-col justify-center items-center rounded-md ">
                    <div className="user-list-content w-full h-1/2 p-4 rounded-md">
                        <h3 className="text-header text-lg font-semibold">
                            User List
                        </h3>
                        ...
                        <ul className="h-full py-2 flex justify-center items-center gap-2 flex-col max-h-[180px] overflow-y-auto scrollbar-hide">
                            {filteredUsers.map(users => {
                                if (
                                    users &&
                                    (users.id === meeting?.created_by_user_id ||
                                        user.id === users.id)
                                ) {
                                ...
                                return (
                                    <li
                                        ...
                                        className={`w-full flex items-center justify-between p-2 border rounded cursor-pointer shadow ${
                                            formData.user_ids.includes(users.id)
                                                ? 'bg-blue-100'
                                                : ''
                                        }`}
                                        onClick={() =>
                                            handleUserSelection(users.id)
                                        }
                                    >
                            ...
                            {formData.user_ids.map(userId => {
                                ...
                                return person ? (
                                    <li
                                        key={person.id}
                                        onClick={() =>
                                            handleUserSelection(person.id)
                                        }
                                        className="user-box w-full flex items-center justify-between p-2 border rounded cursor-pointer shadow"
                                    >
                                        {person.name}
                                        <input
                                            type="checkbox"
                ...
                <div className="user-list-content w-2/3 mr-4 h-full rounded-md shadow p-4">
                    ...
                    >
                        ...
                        {meeting && <p>Created by: {meeting.creator.name}</p>}
                        ...
                        <div className="box-form flex w-full relative">
                            ...
                            <p className="font-semibold text-base absolute left-1/2 -translate-x-1/2 top-9">
                                -
                            </p>
