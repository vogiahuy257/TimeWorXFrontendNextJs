            ...
            <div className="w-1/3 gap-4 flex flex-col justify-center items-center rounded-md ">
                ...
                <div className="user-list-content w-full h-1/2 p-4 rounded-md">
                  ...
                  {filteredUsers.map((users) => {
                    ...
                    if (users && (users.id === meeting?.created_by_user_id || user.id === users.id)) {
                    ...
                    return (
                      <li
                        ...
                        className={`w-full flex items-center justify-between p-2 border rounded cursor-pointer shadow ${
                          formData.user_ids.includes(users.id) ? "bg-blue-100" : ""
                        }`}
                        onClick={() => handleUserSelection(users.id)}
                      >
                        <span>{users.name}</span>
                        <input
                          type="checkbox"
                          checked={formData.user_ids.includes(users.id)}
                          readOnly
                ...
                <div className="user-list-content h-1/2 w-full p-4 rounded-md">
                  <div className="flex gap-2 items-center">
                    ...
                    {formData.user_ids.map((userId) => {
                        ...
                        if (person && (person.id === meeting?.created_by_user_id || user.id === person.id)) {
                          return null // Bỏ qua nếu user.id trùng với created_by_user_id
                        }
                        return person ? (
                        <li key={person.id} onClick={() => handleUserSelection(person.id)} className="user-box w-full flex items-center justify-between p-2 border rounded cursor-pointer shadow">
                            {person.name}
                            <input
                                type="checkbox"
                                checked={formData.user_ids.includes(person.id)}
                                readOnly
                                className="h-4 w-4 rounded-sm text-blue-600"
                            />
                        </li>
