        ...
        const fetchChartData = async () => {
            try {
                const response = await axios.get(
                    `/api/tasks/${project_id}/done`,
                )
                const data = response.data
                const sortedData = data.sort((a, b) => {
                    if (a.status_key === 'verify' && b.status_key !== 'verify')
                        return -1
                    if (a.status_key !== 'verify' && b.status_key === 'verify')
                        return 1
                    return 0
                ...
                {filteredTasks.length > 0 ? (
                    filteredTasks.map((task, index) => (
                        ...
                        >
                            ...
                            <h3 className="w-5/6 break-words whitespace-nowrap overflow-hidden text-ellipsis truncate">
                                {task.task_name}
                            </h3>
                        </button>
                    ))
